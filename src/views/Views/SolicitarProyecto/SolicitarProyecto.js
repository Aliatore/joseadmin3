...
const Lista = (props) => {
  ...
  // const [codTeacher, setCodTeacher] = useState('');
  ...
  //  const setProfesores = () => {
  //    try {
  //      if(data.datosProfesor !== null) {
  //        return data.datosProfesor.map((e, i) => {
  //         return <option key={i} value={`${e.Teachercode}-${e.Name}-${e.Lastname}`}>Nombre: {e.Name}</option>
  //       }) 
  //      }else{
  //        console.log("error")
  //      }
  ...
  //   setCodTeacher(TCode)
  //   console.log("onEvent", TCode)
  //   console.log("onEvent", TName)
  //   console.log("onEvent", TLastname)
  // }
  // const profesores = (e) => {
  //   let userIdProject = e;
  //   setIdCodeProject(userIdProject)
  //   console.log('skere');
  ...
  const solicitar = () => {
    //console.log('');
    //Data a enviar con el axios
    const url = `https://admin.apruebaexamenes.com/Peticion-Project`

    let datosLogin

    datosLogin = {
      Projectid: data.proyecto_solicitado,
      Userid: parseInt(props.datacliente.iduser)
    }
    ...
    .then((response) => {
      if (response.data.code === '000') {
        //DEberia haber un validador qué verifique si está ññegando bien la data o no, revisar registrar usuario.js 
      //console.log(response.data,"respuestaDelApi")
      toast.success(`✓ - Solicitado satisfactoriamente`, {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
        ...
        <MDBContainer>
          ...
          <MDBContainer>
            <MDBModal isOpen={toggle2}>
              <MDBModalHeader style={{textAlign:"center"}}>Solicitar proyecto</MDBModalHeader>
                <MDBModalBody>
                <Form className="pt-2 ml-3">
                  <FormGroup row>
                    <Label xs={12} sm={12} md={12} lg={12} xl={12}
                    for="rol" 
                    style={{fontSize: 18, fontWeight: 'bold', textAlign: 'left'}}
                    >¿Estás seguro de solicitar la realización de este proyecto?
                    </Label>
                    {/* <Col sm={6}>
                      <Input 
                        type="select" 
                        name="rol"
                        value={data.codProfesor}
                        onChange={e => onEventProfesores(e)}
                        required
                      >
                        <option value="">Seleccione el código de profesor</option>
                        {data.datosProfesor !== null && data.datosProfesor !== '' ? 
                           setProfesores()
                          : null
                        }
                      </Input>
              ...
              <MDBModalFooter>
                <MDBBtn style={{textAlign:"left"}} color='danger' onClick={() => setToggle2(false)}>
                  Cancelar
                </MDBBtn>
                <MDBBtn color='success' onClick={() => solicitar()}>
                  Solicitar
                </MDBBtn>
              </MDBModalFooter>
            </MDBModal>
